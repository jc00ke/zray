        ...
        if (media === null) {
          ...
          if (playPromise !== undefined) {
            playPromise
              ...
              .catch(function(err) {
                console.log(
                  "media element with id #" +
                    msg.data.id +
                    " failed to play because of " +
                    err
                );
              });
          } else {
            console.log(
              "HTMLMediaElement.play() does not return a promise in this browser."
            );
          }
          break;
        }
        ...
        Object.defineProperty(TimeRanges.prototype, "asArray", {
          get: function() {
            ...
            for (i = 0; i < this.length; i++) {
              arr.push({ start: this.start(i), end: this.end(i) });
            }
            return arr;
          }
        });
...
(function() {
  ...
  let zray1 = Elm.Main.init({
    node: document.getElementById("video1"),
    flags: flags
  });

  zray1.ports.outbound.subscribe(MediaApp.portHandler);

  let zray2 = Elm.Main.init({
    node: document.getElementById("video2"),
    flags: flags
  });

  zray2.ports.outbound.subscribe(MediaApp.portHandler);
})()
